...
#conn = ibis.connect(database_url)
```

```{python}
# Load the data
jp_df = pl.scan_csv("data/raw/jp_instance.csv", ignore_errors=True)
...
jp_df = jp_df.select(pl.col("date", "trade_id", "country_id", "sitc_id", "hts_id", "naics_id", "district_id", "unit1_id", "unit2_id", "data", "end_use_i", "end_use_e", "qty_1", "qty_2"))
```

```{python}
country.collect().write_database(table_name="countrytable", connection=database_url, if_table_exists="append")
sitc.collect().write_database(table_name="sitctable", connection=database_url, if_table_exists="append")
hts.collect().write_database(table_name="htstable", connection=database_url, if_table_exists="append")
naics.collect().write_database(table_name="naicstable", connection=database_url, if_table_exists="append")
distric.collect().write_database(table_name="districttable", connection=database_url, if_table_exists="append")
unit.collect().write_database(table_name="unittable", connection=database_url, if_table_exists="append")
jp_df.collect().write_database(table_name="jptradedata", connection=database_url, if_table_exists="append")
```

```{python}
import ibis
con = ibis.connect("postgresql://postgres:password@localhost:5432/timescaledb")
